...
for i = 1:n
    ...
    C2{i} = double(data{i,1}{2})./2^16; % YFP (reporter)
    C3{i} = double(data{i,1}{3})./2^16; % CFP (reporter)
    C4{i} = double(data{i,1}{4})./2^16; % mCherry (BAC)
end
...
%% draw line across reporter and tox colonies/lawns
CFPintensity = {}; YFPintensity = {}; mChintensity = {};
xCoords = {}; yCoords = {};

h = figure('color','white');
for i = 1:n
    curImg = imfuse(C3{i},C4{i},'falsecolor'); %merge mcherry and CFP channels
    imshow(curImg);
    ...
    roi1 = drawline %mark border of strains
    x1 = roi1.Position(:,1); y1 = roi1.Position(:,2);
    [xCol{i} yCol{i}] = polyxpoly([x(1),x(2)],[y(1),y(2)],[x1(1),x1(2)],[y1(1),y1(2)]);
...
for i = 1:n 
    ...
    YFPintensity{i} = interp2(double(C2{i}), xCoords{i}, yCoords{i});
    CFPintensity{i} = interp2(double(C3{i}), xCoords{i}, yCoords{i}); 
    mChintensity{i} = interp2(double(C4{i}), xCoords{i}, yCoords{i});
end
...
for i=1:n
    ...
    col{i}.c2 = YFPintensity{i}; col{i}.c2_norm = normalize(col{i}.c2,'range');
    col{i}.c3 = CFPintensity{i}; col{i}.c3_norm = normalize(col{i}.c3,'range');
    col{i}.c4 = mChintensity{i}; col{i}.c4_norm = normalize(col{i}.c4,'range');
    col{i}.micron_per_pixel = micron_per_pixel;
    col{i}.edges = [xCol{i},yCol{i}];
end
